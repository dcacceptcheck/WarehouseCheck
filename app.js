...
const els = {
  ...
  codeStartDate: document.getElementById("codeStartDate"),
...
function readRow(row) {
  ...
  const codeStartDateFromFile = parseExcelDate(
    findValueByExact(row, ["วันผลิต+จำนวนวันที่ต้องไม่เกิน นับจากวันผลิต"]) ??
...
function syncCodeStartDate() {
  ...
  els.codeStartDate.value = firstRow?.codeStartDate ? formatDate(firstRow.codeStartDate) : "";
}
...
function getValidation() {
  ...
  const codeStartDateText = els.codeStartDate.value.trim();

  if (!mallText || !productText || !destinationDateText || !codeStartDateText) {
  ...
  const codeStartDate = parseDateInput(codeStartDateText);
  if (!codeStartDate) {
    return { valid: false, message: "ไม่พบวันที่รหัสจ่ายได้ตั้งแต่วันที่จากฐานข้อมูล", error: true };
  }
  ...
  return { valid: true, destinationDate, codeStartDate, mall, product, rows };
}
...
function renderResults() {
  ...
  validation.rows.forEach((row) => {
    ...
    tr.appendChild(createCell("วันผลิต", formatDate(row.productionDate)));
    tr.appendChild(createCell("เกณฑ์รับสินค้า (วัน)", `${row.criteriaDays.toLocaleString("th-TH")} วัน`));
    tr.appendChild(createCell("วันที่ไม่เกินเกณฑ์ที่ลูกค้ายอมรับ", formatDate(row.codeStartDate)));
