...
function getValidation() {
  ...
  return { valid: true, destinationDate, mall, product, rows };
}
...
function renderResults() {
  ...
  validation.rows.forEach((row) => {
    ...
    const calculatedCodeStartDate = addDays(validation.destinationDate, -row.criteriaDays);
    const codeStartBadge = document.createElement("span");
    codeStartBadge.className = "code-start-date-highlight";
    codeStartBadge.textContent = formatDate(calculatedCodeStartDate);
    tr.appendChild(createCell("รหัสจ่ายได้ตั้งแต่วันที่", codeStartBadge, "code-start-cell"));
